...
class JobCardListView(APIView):
    ...
    def get(self, request):
        # Add server-side pagination with select_related for performance
        queryset = JobCard.objects.select_related(
            "region", "technician", "customer", "job_type", "support_agent"
        ).order_by("-created_at")
        ...
        try:
            paginator.page_size = int(page_size_param) if page_size_param else 25
...
class TopTechniciansView(APIView):
        """
        GET top technicians by completed jobcards count.
        Query params:
          - region_id (optional)
        ...
        def get(self, request):
            ...
            # Build cache key
            cache_key = f"top_techs:{region_id}:{start_date}:{end_date}:{limit}"
            ...
            agg = (
                queryset
                .values("technician__id", "technician__name")
                .annotate(count=Count("id"))
                .order_by("-count")[:limit]
            )
    
            result = [
                {"id": a["technician__id"], "name": a["technician__name"], "count": a["count"]}
                for a in agg
            ]
